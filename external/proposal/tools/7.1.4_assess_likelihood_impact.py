#!/usr/bin/env python3
"""
Assess Likelihood and Impact (Task 7.1.4)

This script loads the risk register, assigns likelihood and impact ratings to each risk,
calculates an overall risk level, and outputs updated JSON and Markdown documentation.
"""

import json
import logging
from datetime import datetime
from pathlib import Path
from typing import Dict, List, Any

# Configure logging
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(levelname)s - %(message)s',
    handlers=[
        logging.FileHandler('tools/7.1.4_assess_likelihood_impact.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

# Define file paths
BASE_SOURCES_PATH = Path("sources")
BASE_DOCS_PATH = Path("docs")

# Input files
INPUT_RISK_REGISTER = BASE_SOURCES_PATH / "7.1.3-risk-register.json"

# Output files
OUTPUT_RISK_REGISTER = BASE_SOURCES_PATH / "7.1.4-risk-register-likelihood-impact.json"
OUTPUT_RISK_REGISTER_MD = BASE_DOCS_PATH / "7.1.4-risk-likelihood-impact.md"

# Likelihood and impact scales
LIKELIHOOD_SCALE = {
    'Rare': 1,
    'Unlikely': 2,
    'Possible': 3,
    'Likely': 4,
    'Almost Certain': 5
}
IMPACT_SCALE = {
    'Low': 1,
    'Moderate': 2,
    'Significant': 3,
    'High': 4,
    'Severe': 5
}
RISK_LEVEL_MATRIX = {
    (1, 1): 'Low', (1, 2): 'Low', (1, 3): 'Low', (1, 4): 'Medium', (1, 5): 'Medium',
    (2, 1): 'Low', (2, 2): 'Low', (2, 3): 'Medium', (2, 4): 'Medium', (2, 5): 'High',
    (3, 1): 'Low', (3, 2): 'Medium', (3, 3): 'Medium', (3, 4): 'High', (3, 5): 'High',
    (4, 1): 'Medium', (4, 2): 'Medium', (4, 3): 'High', (4, 4): 'High', (4, 5): 'Critical',
    (5, 1): 'Medium', (5, 2): 'High', (5, 3): 'High', (5, 4): 'Critical', (5, 5): 'Critical'
}

# Example mappings for likelihood and impact (can be refined per project context)
LIKELIHOOD_MAPPING = {
    'Technical': 'Possible',
    'Methodological': 'Possible',
    'Resource': 'Likely',
    'Impact': 'Unlikely',
    'Timeline': 'Likely'
}
IMPACT_MAPPING = {
    'Technical': 'Significant',
    'Methodological': 'Moderate',
    'Resource': 'High',
    'Impact': 'Severe',
    'Timeline': 'High'
}

def load_risk_register() -> List[Dict[str, Any]]:
    try:
        with INPUT_RISK_REGISTER.open('r', encoding='utf-8') as f:
            data = json.load(f)
            # Support both list and dict structure
            if isinstance(data, dict) and 'risk_register' in data:
                return data['risk_register']
            return data
    except Exception as e:
        logger.error(f"Error loading risk register: {e}")
        return []

def assess_likelihood_impact(risks: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    updated_risks = []
    for risk in risks:
        # Assign likelihood and impact based on category (can be customized)
        likelihood_label = LIKELIHOOD_MAPPING.get(risk['category'], 'Possible')
        impact_label = IMPACT_MAPPING.get(risk['category'], 'Moderate')
        likelihood = LIKELIHOOD_SCALE[likelihood_label]
        impact = IMPACT_SCALE[impact_label]
        risk_level = RISK_LEVEL_MATRIX[(likelihood, impact)]
        
        risk.update({
            'likelihood_label': likelihood_label,
            'likelihood': likelihood,
            'impact_label': impact_label,
            'impact': impact,
            'risk_level': risk_level
        })
        updated_risks.append(risk)
    return updated_risks

def generate_output_files(risks: List[Dict[str, Any]]):
    # Prepare JSON output
    json_output = {
        "metadata": {
            "task": "7.1.4 - Assess Likelihood and Impact",
            "generated_date": datetime.now().isoformat(),
            "description": "Risk register with likelihood, impact, and risk level",
            "total_risks": len(risks)
        },
        "risk_register": risks
    }
    try:
        with OUTPUT_RISK_REGISTER.open('w', encoding='utf-8') as f:
            json.dump(json_output, f, indent=2, ensure_ascii=False)
        logger.info(f"Updated risk register JSON saved to: {OUTPUT_RISK_REGISTER}")
    except IOError as e:
        logger.error(f"Error saving JSON output: {e}")
        raise
    # Generate Markdown output
    md_content = generate_markdown_content(json_output)
    try:
        with OUTPUT_RISK_REGISTER_MD.open('w', encoding='utf-8') as f:
            f.write(md_content)
        logger.info(f"Updated risk register Markdown saved to: {OUTPUT_RISK_REGISTER_MD}")
    except IOError as e:
        logger.error(f"Error saving Markdown output: {e}")
        raise

def generate_markdown_content(data: Dict[str, Any]) -> str:
    md_lines = [
        "# Risk Register: Likelihood & Impact Assessment (Task 7.1.4)",
        "",
        "## 1. Executive Summary",
        "",
        f"This risk register includes likelihood, impact, and risk level for {data['metadata']['total_risks']} risks.",
        "",
        "## 2. Risk Register Table",
        "",
        "| Risk ID | Description | Category | Likelihood | Impact | Risk Level |",
        "|---------|-------------|----------|------------|--------|-----------|"
    ]
    for risk in data['risk_register']:
        md_lines.append(
            f"| {risk['risk_id']} | {risk['description']} | {risk['category']} | "
            f"{risk['likelihood_label']} | {risk['impact_label']} | {risk['risk_level']} |"
        )
    md_lines.extend([
        "",
        "## 3. Risk Details",
        ""
    ])
    for risk in data['risk_register']:
        md_lines.extend([
            f"### {risk['risk_id']}: {risk['description']}",
            "",
            f"- **Category**: {risk['category']}",
            f"- **Likelihood**: {risk['likelihood_label']} ({risk['likelihood']})",
            f"- **Impact**: {risk['impact_label']} ({risk['impact']})",
            f"- **Risk Level**: {risk['risk_level']}",
            f"- **Current Status**: {risk['current_status']}",
            f"- **Risk Owner**: {risk['risk_owner']}",
            f"- **Review Frequency**: {risk['review_frequency']}",
            f"- **Last Review Date**: {risk['last_review_date']}",
            ""
        ])
    md_lines.extend([
        "## 4. Next Steps",
        "",
        "- Prioritize risks (Task 7.1.5)",
        "- Develop mitigation strategies (Task 7.2.1)",
        "- Create contingency plans (Task 7.2.2)",
        "",
        "## 5. Maintenance",
        "",
        "This risk register will be updated as part of ongoing risk management."
    ])
    return "\n".join(md_lines)

def print_summary_to_console(data: Dict[str, Any]):
    print("\n" + "=" * 80)
    print("RISK LIKELIHOOD & IMPACT ASSESSMENT COMPLETED (Task 7.1.4)")
    print("=" * 80)
    print(f"Total risks assessed: {data['metadata']['total_risks']}")
    # Count by risk level
    levels = {}
    for risk in data['risk_register']:
        level = risk['risk_level']
        if level not in levels:
            levels[level] = 0
        levels[level] += 1
    print("\nRisks by Level:")
    for level, count in levels.items():
        print(f"  • {level}: {count} risks")
    print(f"\nDetailed register saved to: {OUTPUT_RISK_REGISTER}")
    print(f"Markdown documentation saved to: {OUTPUT_RISK_REGISTER_MD}")
    print("=" * 80)

def main():
    logger.info("Starting Task 7.1.4: Assess Likelihood and Impact")
    risks = load_risk_register()
    if not risks:
        logger.error("No risks found in register.")
        return
    updated_risks = assess_likelihood_impact(risks)
    generate_output_files(updated_risks)
    # Print summary
    with OUTPUT_RISK_REGISTER.open('r', encoding='utf-8') as f:
        data = json.load(f)
    print_summary_to_console(data)
    logger.info("Task 7.1.4: Assess Likelihood and Impact - COMPLETED SUCCESSFULLY")

if __name__ == "__main__":
    main() 